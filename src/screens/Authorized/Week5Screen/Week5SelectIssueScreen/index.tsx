...
type Props = NativeStackScreenProps<Week5StackParamsList, 'Week5SelectIssue'>;

type RouteProps = Props['route'];
type NavigationProps = Props['navigation'];

function Week5SelectIssueScreen() {
  const {goBack, navigate} = useNavigation<NavigationProps>();
  const theme = useTheme();

  const goBackHome = () => goBack();

  const goBackWeek5Home = () => navigate('Week5Home');
  ...
  const renderRightAction = () => {
    return (
      <TouchableOpacity style={styles.nextButton} onPress={goBackWeek5Home}>
...
export default Week5SelectIssueScreen;
